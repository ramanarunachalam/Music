    ...
    $.getJSON(url, function(search_obj) {
        ...
        for (var category in search_obj) {
            ...
            data_list.forEach(function (data_item, data_index) {
                var data_doc = { "id" : data_id, "title" : data_item.N, "aka" : data_item.A, "href" : data_item.H, "category" : category };
                engine.add(data_doc);
                data_id += 1;
            });
        }
...
function frame_search_init(search_data) {
    const query_string = window.location.search;
    const url_params = new URLSearchParams(query_string);
    const search_word = url_params.get('word')
    var engine = window.parent.carnatic_search_engine;
    var search_options = { prefix: true, combineWith: 'AND', fuzzy: term => term.length > 3 ? 0.3 : null };
    var results = engine.search(search_word, search_options);
    var ICON_DICT = { 'song' : 'music-note-beamed', 'artist' : 'person-fill', 'composer' : 'person-lines-fill', 'raga' : 'music-note-list', 'type' : 'tag' };
    var item_list = [];
    results.forEach(function (result_item, result_index) {
        var item = { 'T' : result_item.category, 'C' : result_item.category.toUpperCase(), 'I' : ICON_DICT[result_item.category], 'H' : result_item.href, 'N' : result_item.title };
        item_list.push(item);
    });
    var new_item_list = item_list.slice(0, 25);
    var item_data = { "items" : new_item_list };
    render_template(item_data);
}
