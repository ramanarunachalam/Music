...
function get_search_results(search_word, search_options, item_list, id_list) {
    var icon_dict = window.parent.CARNATIC_ICON_DICT;
    var search_engine = window.parent.carnatic_search_engine;
    ...
    if (results.length > 0) {
        ...
        results.forEach(function (result_item, result_index) {
            if (!id_list.has(result_item.id)) {
                var pop = ((900 * result_item.score) / max_score) + (0.1 * result_item.pop);
                var item = { 'T' : result_item.category, 'C' : result_item.category.toUpperCase(), 'I' : icon_dict[result_item.category], 'H' : result_item.href, 'N' : result_item.title, 'P' : pop };
                item_list.push(item);
...
function frame_search_init(search_data) {
    ...
    var item_list = [];
    var id_list = new Set();
    // var search_options = { prefix: true, combineWith: 'AND' }
    var search_options = { prefix: true, combineWith: 'AND', fuzzy: term => term.length > 3 ? 0.1 : null };
    var first_list = get_search_results(search_word, search_options, item_list, id_list);
    var search_options = { prefix: true, combineWith: 'AND', fuzzy: term => term.length > 3 ? 0.3 : null };
    var second_list = get_search_results(search_word, search_options, item_list, id_list);
