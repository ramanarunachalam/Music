...
function render_nav_template(data) {
    ...
    for (var k = 0; k < letter_list.length; k++) {
        var item_list = letter_list[k]['items'] 
...
function load_content_data(category, name) {
    ...
    $.getJSON(url, function(video_data) {
        ...
        render_card_template('#page-stats-template', '#PAGE_STATS', video_data);
...
function load_search_data() {
    ...
    render_card_template('#page-stats-template', '#PAGE_STATS', item_data);
...
function play_notes(notes) {
    var note_map = { 'S' : 'C', 'S1' : 'C', 'R1' : 'Db', 'R2' : 'D', 'G1' : 'D', 'R3' : 'Eb', 'G2' : 'Eb', 'G3' : 'E', 'M1' : 'F', 'M2' : 'Gb', 'P' : 'G', 'D1' : 'Ab', 'D2' : 'A', 'N1' : 'A', 'D3' : 'Bb', 'N2' : 'Bb', 'N3' : 'B', 'S2' : 'B' };
    var note_list = notes.split(' ');
    for (var i = 0; i < note_list.length; i++) {
        var note = note_map[note_list[i]];
        play_note(note);
        var key_div = '#note' + note;
        /*
        var key = $(key_div).css("background-color", "red");
        */
        sleep(2);
    }
}
